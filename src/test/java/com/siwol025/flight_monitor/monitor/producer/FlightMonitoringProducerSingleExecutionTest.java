 ...
 * 설정값(PT60S)이 placeholder 해석을 거쳐 실제 {@link LockConfiguration} 으로 ShedLock 에 전달되는지를
...
@TestPropertySource(properties = {
        ...
        "monitor.produce.lock-at-least-for=PT55S",
        "monitor.produce.lock-at-most-for=PT60S"
})
class FlightMonitoringProducerSingleExecutionTest {
    ...
    void 프로듀서_리더선출_락설정_전달됨() {
        ...
        assertThat(config.getLockAtLeastFor())
                .as("lockAtLeastFor placeholder(PT55S)가 해석되어 전달되어야 한다")
                .isEqualTo(Duration.ofSeconds(55));
        assertThat(config.getLockAtMostFor())
                .as("lockAtMostFor placeholder(PT60S)가 해석되어 전달되어야 한다")
